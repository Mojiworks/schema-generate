...
func Output(w io.Writer, g *Generator, pkg string, allowCodeGeneration bool) {
	structs := g.Structs
	aliases := g.Aliases
	...
	fmt.Fprintf(w, "// (allowCodeGeneration: %s)", codeGen)
	fmt.Fprintln(w)
	...
	for _, k := range getOrderedStructNames(structs) {
		s := structs[k]
		if allowCodeGeneration && s.GenerateCode {
